...
def load_classes(path_or_list: str):
    if Path(path_or_list).exists():
        return [c.strip() for c in Path(path_or_list).read_text().splitlines() if c.strip()]
...
def convert(
    ...
    height: Optional[int] = typer.Option(None, "--height", help="Image height for YOLO"),
    classes: str = typer.Option(..., "-c", help="Path to classes file or comma separated list"),
    ...
    for ann in anns:
        out_file = out_dir / (ann.filename + {"yolo": ".txt", "voc": ".xml"}.get(to_format, ""))
